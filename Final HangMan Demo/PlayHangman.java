...
public class PlayHangman{
  ...
  */
  ...
  private static int customDifficulty(){
    ...
    while(true){
      ...
      System.out.println();
      if(customLives > 0){     // make sure no negative integer is passed in
        System.out.println();
        return customLives;
      }
      else{
  ...
  */
  ...
  public static int getWordIndex(int totalWords){
    if(Hangman.gameCount%totalWords == 0 && Hangman.gameCount != 0){
      clearWords(totalWords);
    }
    while(true){
      int randomIndex = (int)(Math.random()*totalWords);
      if(Hangman.wordUsed[randomIndex] == false){
        Hangman.wordUsed[randomIndex] = true;
        return randomIndex;
      }
    }
  }
  ...
  */
  ...
  /**
  Method to print out information about current game including the hidden word,
  lives remaining, and the letters used so far.

  @param hiddenWord is printed to standard output.
  @param lives is printed to standard output.
  @param lettersUsed is printed to standard output.

  */

  public static void printUI(StringBuilder hiddenWord , int lives, String lettersUsed){
    System.out.println();
    System.out.printf("%-30s %30s%n", "Word: " + hiddenWord,
    "Lives remaining: " + lives);
    System.out.println();
    System.out.printf("Letters used: %-30s%n" , lettersUsed);
    System.out.println();
    System.out.println();
  }
}
